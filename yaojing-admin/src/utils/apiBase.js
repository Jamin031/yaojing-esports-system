...
const LOGIN_ROUTE_SUFFIX = ['auth', 'login'].join('/');
...
export function isLoginRequestURL(url, apiConfig = resolveApiConfig()) {
  const normalized = String(resolveApiRequestURL(url, apiConfig) || '')
    .trim()
    .toLowerCase();

  if (!normalized) return false;
  return normalized.endsWith(`/${LOGIN_ROUTE_SUFFIX}`);
}
