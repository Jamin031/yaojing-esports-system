...
const PERMISSION_SCHEMAS = {
  ...
  fields: [
    ...
    { key: 'orders:order_remark', name: '订单-订单备注字段' },
    { key: 'orders:problem_remark', name: '订单-问题备注字段' },
    { key: 'orders:store_commission', name: '订单-网吧分成字段' },
    { key: 'orders:play_shop_commission', name: '订单-陪玩店分成字段' },
    { key: 'orders:platform_commission', name: '订单-ESPORTS CLUB分成字段' },
...
const ROLE_DEFAULTS = {
  ...
  customer_service: {
    ...
    pages: ['orders:view', 'problem_orders:view', 'notifications:view'],
    buttons: ['orders:change_status', 'orders:edit_remark', 'orders:problem_save', 'orders:problem_complete'],
    fields: [
      'orders:contact',
      'orders:customer_nickname',
      'orders:info',
